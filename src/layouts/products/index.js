...
function Products() {
    ...
    const categories = useSelector(state => state.category.data.filter(cate => cate.active).sort((a, b) => a.id - b.id));
    const [selectCategory, setSelectCategory] = useState(0);
    const brands = useSelector(state => {
        if (selectCategory === 0) {
            return state.brand.data.filter(b => b.active).sort((a, b) => a.id - b.id);
        } else {
            return state.brand.data.filter(b => b.active).filter(item => item.category.id === selectCategory).sort((a, b) => a.id - b.id);
        }
    });
    ...
    const onSubmit = async (e) => {
        ...
        if (formControl.title === "Update") {
            ...
            dataUpdate.category_id = brands.find(b => b.id === dataUpdate.brand_id).category.id;
        ...
        data.category_id = brands.find(b => b.id === data.brand_id).category.id;
            ...
            >
                ...
                <DialogContent>
                    ...
                    <FormControl fullWidth style={{ marginTop: 12 }}>
                        <InputLabel id="demo-simple-select-label-brand" style={{ height: 32 }}>Type</InputLabel>
                        <Select
                        ...
                        >
                            {brands.map((b) => (
                                <MenuItem key={b.id} value={b.id}>{b.name}{`(${b.category.name})`}</MenuItem>
                            ))}
                        </Select>
                    </FormControl>
                                ...
                                >
                                    ...
                                    <MDBox>
                                        <Box sx={{ minWidth: 100, display: "flex", gap: 1 }}>
                                            ...
                                            <FormControl fullWidth>
                                                <InputLabel style={{ color: "white", height: 32 }} id="demo-simple-select-label">Type</InputLabel>
                                                <Select
                                                    labelId="demo-simple-select-label"
                                                    id="demo-simple-select"
                                                    label="Brand"
                                                ...
                                                >
                                                    <MenuItem value={0}>All</MenuItem>
                                                    {brands.map(brand => (
                                                        <MenuItem key={brand.id} value={brand.id}>{brand.name}{`(${brand.category.name})`}</MenuItem>
                                                    ))}
                                                </Select>
                                            </FormControl>
                                        </Box>
